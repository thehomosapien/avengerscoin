...
//   } else {
 ...
 //	std::cout << "Modify this line into your AvengersCoin configuration file as is:  " << std::endl;
 //	std::cout << "const char GENESIS_COINBASE_TX_HEX[] = \"" << tx_hex << "\";" << std::endl;
//   }
//   return;
// }
...
void renameDataDir() {
  std::string avengerscoinDir = Tools::getDefaultDataDirectory();
  boost::filesystem::path avengerscoinDirPath(avengerscoinDir);
  if (boost::filesystem::exists(avengerscoinDirPath)) {
    return;
  }

  std::string dataDirPrefix = avengerscoinDir.substr(0, avengerscoinDir.size() + 1 - sizeof(CRYPTONOTE_NAME));

  boost::filesystem::path cediDirPath(dataDirPrefix + "BXC");
  if (boost::filesystem::exists(cediDirPath)) {
    boost::filesystem::rename(cediDirPath, avengerscoinDirPath);
  } else {
    boost::filesystem::path BcediDirPath(dataDirPrefix + "Bcedi");
    if (boost::filesystem::exists(boost::filesystem::path(BcediDirPath))) {
		boost::filesystem::rename(BcediDirPath, avengerscoinDirPath);
    }
  }
}
