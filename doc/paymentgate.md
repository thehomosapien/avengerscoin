...
data-dir=/srv/avengerscoin	    # Where blockchain data is stored. Defaults to /home/user/.AvengersCoin
bind-address=0.0.0.0	    # Use server IP or 127.0.0.1
bind-port=8070		    # Port
rpc-user=                   # Auth user for RPC service
rpc-password=               # Password for RPC service
server-root=/srv/avengerscoin  # Working directory
...
```ini
container-file=/etc/avengerscoin/container.iwallet
bind-port=8070
...
AvengersCoin `walletd` daemon allows to send arbitraty data within transaction extra. It can be any text string.
